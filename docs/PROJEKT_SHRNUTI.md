...
- **v24.4: Korekční smyčka heartbeat**: Přidán inject node `solar_corr_inject_20s` (repeat 20s, startup delay 30s) do flow `nabijeni-auta-slunce.json`. Řeší problém: po NR restartu `server-state-changed` trigger (`outputInitially:false`) neposlal event pokud boolean byl už ON → korekční smyčka se nespustila. Fix: inject každých 20s kontroluje boolean a spouští korekční řetěz. Trigger nastaven na `outputInitially:true` pro extra robustnost.

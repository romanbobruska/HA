...
## v25.127: Yale zámek — EMPIRICKY potvrzeno (řízený test) + UC5 přítomnost + UC6 grace (2026-06-09)

**Řízený živý test (uživatel u dveří):** Potvrzeno, že na `lock.hlavni_dvere` **`lock.unlock` i `lock.open` z HA VŽDY fyzicky otevřou dveře** (stáhne se i západka → dveře nejdou dovřít/zamknout → zásek). Odemčení v **Yale appce dveře NEOTEVÍRÁ** (jen závora — ověřeno senzorem). Příčina = chování Yale integrace v HA (issue #146352), případně kalibrace zámku (latch-retract se nastavuje při kalibraci v Yale appce). **Pravidlo nejvyšší priority: automatika smí volat jen `lock.lock`.**

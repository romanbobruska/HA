...
| Soubor | Co dělá |
|--------|---------|
| `fve-orchestrator.json` | Plánovač módů v23 na 12h. **v23**: Multi-hour balancing — `balancingHoursUsed < balancingForceStopHours` (default 2h). **Per-hour solar check**: balancing se plánuje JEN do hodin s dostatečným solárním výkonem (`forecastPerHour[h] >= balancing_min_solar_kwh`, default 2 kWh). Fallback na curve estimate. **Rozhodnutí o akci**: trackuje `balancing_started_at` global. Force-stop po max době NEROUTE na Normal — místo toho nastaví `msg.forceStopBalancing=true` a routuje na BALANCOVÁNÍ Logic, aby mohl odeslat service call pro `last_pylontech_balanced`. Clearuje `balancing_completed` při opuštění Balancování. PRIORITA 0.5: `_balCanAssign` = balancingNeeded && hoursUsed < max && elapsed < max. **Solar-first**: solární hodiny = nabíjí; levné hodiny = grid nabíjení JEN pokud žádné solární hodiny není k dispozici. |

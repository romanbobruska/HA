...
sudo -n python3 /tmp/HA/scripts/deploy_copy_ha.py || true
...
if [ ! -d "$NODERED_DIR" ]; then
    ...
    FLOWS_DIR="$REPO_DIR/node-red/flows" OUTPUT_FILE="$NODERED_DIR/flows.json" \
        sudo -n -E python3 /tmp/HA/scripts/deploy_merge_flows.py 2>&1
    if [ $? -eq 0 ]; then
    ...
    OUTPUT_FILE="$NODERED_DIR/flows.json" sudo -n -E python3 /tmp/HA/scripts/deploy_audit_groups.py 2>&1 || true
fi
